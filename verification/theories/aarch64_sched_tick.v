 ...
 * flag, and the quantum update path used by the AArch64 runtime hooks.
 *
 ...
 *   - quantum updates reject zero and preserve positivity on success
 *
...
Definition a64_effective_quantum (s : A64SchedState) : nat :=
  Nat.max 1 (a64_quantum s).

Definition a64_timer_tick (s : A64SchedState) : A64SchedState :=
  let total' := S (a64_tick_total s) in
  let quantum := a64_effective_quantum s in
  let pos := total' mod quantum in
  let pending' :=
...
Definition a64_set_quantum (ticks : nat) (s : A64SchedState) : option A64SchedState :=
...
Proof.
  ...
  simpl in Hboundary.
  destruct (Nat.eqb (S (a64_tick_total s) mod a64_effective_quantum s) 0) eqn:Hcase.
...
Lemma a64_set_quantum_rejects_zero :
  forall s : A64SchedState,
    a64_set_quantum 0 s = None.
Proof.
  intros s.
  unfold a64_set_quantum.
  simpl.
  reflexivity.
Qed.

Lemma a64_set_quantum_success_preserves_positive :
  forall ticks s s',
    a64_set_quantum ticks s = Some s' ->
    0 < a64_quantum s' /\ a64_quantum s' = ticks.
Proof.
  intros ticks s s' Hset.
  unfold a64_set_quantum in Hset.
...
Proof.
  ...
  - split.
    ...
    + split.
      * exact (a64_set_quantum_rejects_zero s).
      * intros ticks s' Hset.
        exact (a64_set_quantum_success_preserves_positive ticks s s' Hset).
Qed.
...
(* Code-model trace:
   ...
   - kernel/src/arch/aarch64_virt.rs::scheduler_tick_backend_set_quantum
   - kernel/src/scheduler/quantum_scheduler.rs::on_timer_tick
   - kernel/src/scheduler/quantum_scheduler.rs::maybe_reschedule
*)
