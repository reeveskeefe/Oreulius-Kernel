...
pub fn run_deterministic_tests() -> Vec<(&'static str, bool, String)> {
    ...
    if got == expected_empty {
        results.push(("sha512_empty", true, "OK".to_string()));
    } else {
        results.push(("sha512_empty", false, format!("got {}", to_hex(&got))));
    }
    ...
    let got2 = sha512(b"abc");
    if got2 == expected_abc {
        results.push(("sha512_abc", true, "OK".to_string()));
    } else {
        results.push(("sha512_abc", false, format!("got {}", to_hex(&got2))));
    }
