...
impl NvmeCqe {
    ...
    pub fn status(&self) -> u16 { (self.phase_status >> 1) & 0x7FFF }
}
...
#[repr(C, align(4096))]
struct AdminSqBuf { data: [NvmeSqe; NVME_ADMIN_QUEUE_DEPTH] }
#[repr(C, align(4096))]
struct AdminCqBuf { data: [NvmeCqe; NVME_ADMIN_QUEUE_DEPTH] }
#[repr(C, align(4096))]
struct IoSqBuf    { data: [NvmeSqe; NVME_IO_QUEUE_DEPTH] }
#[repr(C, align(4096))]
struct IoCqBuf    { data: [NvmeCqe; NVME_IO_QUEUE_DEPTH] }

static mut NVME_ADMIN_SQ: AdminSqBuf = AdminSqBuf { data: [NvmeSqe { cdw0:0,nsid:0,cdw2:0,cdw3:0,mptr:0,prp1:0,prp2:0,cdw10:0,cdw11:0,cdw12:0,cdw13:0,cdw14:0,cdw15:0 }; NVME_ADMIN_QUEUE_DEPTH] };
static mut NVME_ADMIN_CQ: AdminCqBuf = AdminCqBuf { data: [NvmeCqe { dw0:0,dw1:0,sq_hd:0,sq_id:0,cid:0,phase_status:0 }; NVME_ADMIN_QUEUE_DEPTH] };
static mut NVME_IO_SQ:    IoSqBuf    = IoSqBuf    { data: [NvmeSqe { cdw0:0,nsid:0,cdw2:0,cdw3:0,mptr:0,prp1:0,prp2:0,cdw10:0,cdw11:0,cdw12:0,cdw13:0,cdw14:0,cdw15:0 }; NVME_IO_QUEUE_DEPTH] };
static mut NVME_IO_CQ:    IoCqBuf    = IoCqBuf    { data: [NvmeCqe { dw0:0,dw1:0,sq_hd:0,sq_id:0,cid:0,phase_status:0 }; NVME_IO_QUEUE_DEPTH] };

/// PRP list for multi-page transfers (up to 128 KiB = 32 pages).
#[repr(C, align(4096))]
struct PrpList { entries: [u64; NVME_MAX_PRP_ENTRIES] }
static mut NVME_PRP_LIST: PrpList = PrpList { entries: [0u64; NVME_MAX_PRP_ENTRIES] };
...
impl NvmeController {
    pub fn new(mmio_base: usize, pci: PciDevice) -> Self {
        NvmeController {
            mmio_base, pci, initialised: false,
            block_size: 512, num_blocks: 0, ns1_id: 1,
            dstrd: 4,
            admin_sq_tail: 0, admin_cq_head: 0, admin_cq_phase: true,
            io_sq_tail: 0, io_cq_head: 0, io_cq_phase: true,
            next_cid: 1,
        }
    }
    ...
    fn delay(&self) {
        for _ in 0..50_000u32 { unsafe { core::arch::asm!("nop"); } }
    }
    ...
    unsafe fn poll_admin_cq(&mut self, cid: u16) -> u16 {
        for _ in 0..2_000_000u32 {
            ...
            if cqe.phase() == self.admin_cq_phase {
                ...
                if self.admin_cq_head == 0 { self.admin_cq_phase = !self.admin_cq_phase; }
                self.ring_admin_cq();
                if found_cid == cid { return status; }
            }
        }
        0xFFFF // timeout
    }
    ...
    unsafe fn poll_io_cq(&mut self, cid: u16) -> u16 {
        for _ in 0..2_000_000u32 {
            ...
            if cqe.phase() == self.io_cq_phase {
                ...
                if self.io_cq_head == 0 { self.io_cq_phase = !self.io_cq_phase; }
                self.ring_io_cq();
                if found_cid == cid { return status; }
            }
        }
        ...
        unsafe {
            ...
            if cc & NVME_CC_EN != 0 {
                ...
                for _ in 0..1_000_000u32 {
                    if self.read32(NVME_CSTS) & NVME_CSTS_RDY == 0 { break; }
                }
            }

            // Set Admin Queue Attributes: 63 entries each (0-indexed = 64)
            self.write32(NVME_AQA,
                ((NVME_ADMIN_QUEUE_DEPTH as u32 - 1) << 16) |
                (NVME_ADMIN_QUEUE_DEPTH as u32 - 1));
            ...
            let new_cc = NVME_CC_EN | NVME_CC_CSS_NVM | NVME_CC_MPS_4K
    ...
    pub fn read_sectors(&mut self, lba: u64, sector_count: u32, buf: &mut [u8]) -> bool {
        if !self.initialised { return false; }
        let byte_count = sector_count as usize * self.block_size as usize;
        if buf.len() < byte_count { return false; }
        ...
        unsafe {
            ...
            if st != 0 {
                crate::serial_println!("[NVMe] READ LBA={} count={} status=0x{:X}", lba, sector_count, st);
                return false;
            }
        }
        true
    ...
    pub fn write_sectors(&mut self, lba: u64, sector_count: u32, buf: &[u8]) -> bool {
        ...
        unsafe {
            ...
            let st = self.poll_io_cq(cid_ret);
            if st != 0 {
                crate::serial_println!("[NVMe] WRITE LBA={} count={} status=0x{:X}", lba, sector_count, st);
                return false;
            }
        }
        true
...
pub fn init(pci_devices: &[PciDevice]) {
    for &dev in pci_devices {
        if dev.class_code != PCI_CLASS_STORAGE || dev.subclass != PCI_SUBCLASS_NVME { continue; }
