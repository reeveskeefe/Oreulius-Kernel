...
static mut HDA_CORB: HdaCorbBuf = HdaCorbBuf { data: [0u32; HDA_CORB_SIZE] };
...
static mut HDA_RIRB: HdaRirbBuf = HdaRirbBuf { data: [0u64; HDA_RIRB_SIZE] };
...
static mut HDA_BDL: HdaBdl = HdaBdl {
    entries: [HdaBdlEntry { addr_lo: 0, addr_hi: 0, length: 0, ioc: 0 }; HDA_BDL_ENTRIES],
};
...
static mut HDA_PCM: HdaPcmBuf = HdaPcmBuf { data: [[0u8; HDA_PERIOD_BYTES]; HDA_BDL_ENTRIES] };
...
impl HdaController {
    pub fn new(mmio_base: usize, pci: PciDevice) -> Self {
        HdaController { mmio_base, pci, initialised: false, corb_wp: 0, rirb_rp: 0, dac_cad: 0, dac_nid: 0 }
    }
    ...
    pub fn init(&mut self) -> bool {
        unsafe {
            ...
            for _ in 0..200_000u32 {
                if self.read16(HDA_STATESTS) & 0x01 != 0 { break; }
            }
            ...
            for cad in 0..15u8 {
                ...
                for fg in fg_start..(fg_start + fg_count) {
                    let ftype = self.send_verb(cad, fg, HDA_VERB_GET_PARAM, HDA_PARAM_FUNC_GRP_TYPE as u32);
                    if ftype & 0xFF == 0x01 {
    ...
    fn setup_output_stream(&mut self, cad: u8, fg: u8) {
        unsafe {
            ...
            for nid in wgt_start..(wgt_start + wgt_count) {
                let cap = self.send_verb(cad, nid, HDA_VERB_GET_PARAM, HDA_PARAM_AUDIO_WIDGET as u32);
                ...
                if wtype == 0x00 && dac_nid == 0 {
                    ...
                    let _ = self.send_verb(cad, nid, HDA_VERB_SET_AMP_GAIN,
                                          (1 << 15) | (1 << 13) | (1 << 12) | 0x7F);
    ...
    pub fn set_volume(&mut self, level: u8) {
        if self.dac_nid == 0 { return; }
        let gain = (level as u32) >> 1; // 0-255 → 0-127
        let payload = (1 << 15)        // Output amp
                    | (1 << 13)        // Left channel
                    | (1 << 12)        // Right channel
                    | (gain & 0x7F);
        let _ = self.send_verb(self.dac_cad, self.dac_nid,
                               HDA_VERB_SET_AMP_GAIN, payload);
    }
...
static mut AC97_BDL: Ac97Bdl = Ac97Bdl {
    entries: [Ac97BdlEntry { addr: 0, count: 0, flags: 0 }; AC97_BDL_ENTRIES],
};
...
impl Ac97Controller {
    pub fn new(pci: PciDevice) -> Option<Self> {
        ...
        if bar0 == 0 || bar1 == 0 { return None; }
        // I/O BARs have bit 0 set
        if bar0 & 1 == 0 || bar1 & 1 == 0 { return None; }
        Some(Ac97Controller {
    ...
    fn delay(&self) {
        for _ in 0..10_000u32 { unsafe { core::arch::asm!("nop"); } }
    }
    ...
    pub fn init(&mut self) -> bool {
        unsafe {
            ...
            crate::serial_println!("[AC97] Controller initialised (NAM=0x{:04X} NABM=0x{:04X})",
                                   self.nam_base, self.nabm_base);
        }
    ...
    pub fn start(&self) {
        unsafe { self.nabm_write8(NABM_POCTLB, NABM_CTL_RPBM | NABM_CTL_IOCE); }
    }

    pub fn stop(&self) {
        unsafe { self.nabm_write8(NABM_POCTLB, 0); }
    }
    ...
    pub fn set_volume(&self, level: u8) {
        ...
        unsafe { self.nam_write16(NAM_MASTER_VOL, val); }
    }
