...
#[repr(u8)]
pub enum Whence { Set = 0, Cur = 1, End = 2 }
...
impl WasiCtx {
    ...
    pub fn add_preopen(&mut self, path: &[u8]) -> bool {
        if self.preopen_cnt >= MAX_PREOPENS { return false; }
    ...
    fn get_fd(&self, fd: Fd) -> Option<&OpenFd> {
        ...
        if o.kind == FdKind::Closed { return None; }
        Some(o)
    }
...
pub fn args_sizes_get(ctx: &WasiCtx, mem: &mut [u8], argc_ptr: u32, argv_buf_size_ptr: u32) -> Errno {
    let ap = argc_ptr as usize;
    let sp = argv_buf_size_ptr as usize;
    if ap + 4 > mem.len() || sp + 4 > mem.len() { return Errno::Fault; }
...
pub fn environ_sizes_get(_ctx: &WasiCtx, mem: &mut [u8], cnt_ptr: u32, buf_size_ptr: u32) -> Errno {
    ...
    if cp + 4 > mem.len() || bp + 4 > mem.len() { return Errno::Fault; }
...
pub fn clock_time_get(_ctx: &WasiCtx, mem: &mut [u8], _clock_id: u32, _precision: u64, ts_ptr: u32) -> Errno {
    let p = ts_ptr as usize;
    if p + 8 > mem.len() { return Errno::Fault; }
    // Read PIT ticks and convert to nanoseconds (PIT fires at ~1000 Hz → 1 ms = 1_000_000 ns).
    let ticks = unsafe { crate::pit::get_ticks() };
...
pub fn fd_close(ctx: &mut WasiCtx, fd: Fd) -> Errno {
    if fd < 3 { return Errno::Notsup; }  // can't close stdin/stdout/stderr
    match ctx.get_fd_mut(fd) {
        None => Errno::Badf,
        Some(o) => { *o = OpenFd::closed(); Errno::Success }
    }
}
...
pub fn fd_fdstat_get(ctx: &WasiCtx, mem: &mut [u8], fd: Fd, stat_ptr: u32) -> Errno {
    ...
    if p + stat_size > mem.len() { return Errno::Fault; }
    let ft = match o.kind {
...
pub fn fd_filestat_get(ctx: &WasiCtx, mem: &mut [u8], fd: Fd, stat_ptr: u32) -> Errno {
    ...
    let st = FileStat {
        dev: 0, ino: fd as u64, filetype: ft,
        _pad: [0; 7], nlink: 1, size: o.size, atim: 0, mtim: 0, ctim: 0,
    };
    unsafe {
...
pub fn fd_prestat_get(ctx: &WasiCtx, mem: &mut [u8], fd: Fd, prestat_ptr: u32) -> Errno {
    ...
    if idx >= ctx.preopen_cnt { return Errno::Badf; }
    let p = prestat_ptr as usize;
    if p + core::mem::size_of::<Prestat>() > mem.len() { return Errno::Fault; }
    let ps = Prestat {
        tag: 0,
        _pad: [0; 3],
...
pub fn fd_prestat_dir_name(ctx: &WasiCtx, mem: &mut [u8], fd: Fd, path_ptr: u32, path_len: u32) -> Errno {
    let idx = fd as usize - std_fd::PREOPEN_START as usize;
    if idx >= ctx.preopen_cnt { return Errno::Badf; }
    let src_len = ctx.preopens[idx].name_len as usize;
    let dst_len = path_len as usize;
    let p = path_ptr as usize;
    if p + dst_len > mem.len() { return Errno::Fault; }
...
pub fn fd_read(ctx: &mut WasiCtx, mem: &mut [u8], fd: Fd,
               iovs_ptr: u32, iovs_len: u32, nread_ptr: u32) -> Errno {
    if fd as usize >= MAX_FDS { return Errno::Badf; }
    let kind = ctx.fds[fd as usize].kind;
    let nread_p = nread_ptr as usize;
    if nread_p + 4 > mem.len() { return Errno::Fault; }
    ...
    match kind {
        FdKind::Stdin => {
            ...
            for i in 0..iovs_len as usize {
                ...
                if iov_off + 8 > mem.len() { break; }
                let buf_ptr = u32::from_le_bytes(mem[iov_off..iov_off + 4].try_into().unwrap_or([0;4])) as usize;
                let buf_len = u32::from_le_bytes(mem[iov_off + 4..iov_off + 8].try_into().unwrap_or([0;4])) as usize;
                if buf_ptr + buf_len > mem.len() { return Errno::Fault; }
...
pub fn fd_write(ctx: &mut WasiCtx, mem: &mut [u8], fd: Fd,
                iovs_ptr: u32, iovs_len: u32, nwritten_ptr: u32) -> Errno {
    if fd as usize >= MAX_FDS { return Errno::Badf; }
    let kind = ctx.fds[fd as usize].kind;
    let nw_p = nwritten_ptr as usize;
    if nw_p + 4 > mem.len() { return Errno::Fault; }
...
pub fn fd_seek(ctx: &mut WasiCtx, mem: &mut [u8], fd: Fd,
               offset: i64, whence: u8, newoffset_ptr: u32) -> Errno {
    let o = match ctx.get_fd_mut(fd) {
        None => return Errno::Badf,
        Some(o) => o,
    };
    if o.kind != FdKind::File { return Errno::Spipe; }
...
pub fn path_open(ctx: &mut WasiCtx, _mem: &mut [u8],
                 _dirfd: Fd, _dirflags: u32,
                 path: &[u8], _oflags: u16, _rights: u64,
                 _rights_inheriting: u64, _fdflags: u16,
                 opened_fd_ptr: &mut Fd) -> Errno {
...
pub fn fd_readdir(ctx: &mut WasiCtx, mem: &mut [u8], fd: Fd,
                  buf_ptr: u32, buf_len: u32, cookie: u64, bufused_ptr: u32) -> Errno {
    let _ = (fd, cookie);
    let p = buf_ptr as usize;
    let plen = buf_len as usize;
    let bp = bufused_ptr as usize;
    if bp + 4 > mem.len() { return Errno::Fault; }
    if p + plen > mem.len() { return Errno::Fault; }
...
pub fn path_filestat_get(ctx: &WasiCtx, mem: &mut [u8],
                         _dirfd: Fd, _flags: u32, path: &[u8], stat_ptr: u32) -> Errno {
    let p = stat_ptr as usize;
    let sz = core::mem::size_of::<FileStat>();
    if p + sz > mem.len() { return Errno::Fault; }
    if let Some(key) = core::str::from_utf8(path).ok().and_then(|s| crate::fs::FileKey::new(s).ok()) {
        let data_len = crate::fs::kernel_read_bytes(&key)
            .map(|d| d.len() as u64)
            .unwrap_or(0);
        let ft = if data_len > 0 { filetype::REGULAR_FILE } else { filetype::DIRECTORY };
        let st = FileStat {
            dev: 0, ino: 0, filetype: ft, _pad: [0; 7],
            nlink: 1, size: data_len, atim: 0, mtim: 0, ctim: 0,
        };
        unsafe {
            let src = &st as *const FileStat as *const u8;
...
pub fn random_get(ctx: &mut WasiCtx, mem: &mut [u8], buf_ptr: u32, buf_len: u32) -> Errno {
    ...
    if p + l > mem.len() { return Errno::Fault; }
    let mut i = 0usize;
    while i + 8 <= l {
        let v = ctx.prng_next();
...
///
/// Simplified: we only handle clock-based subscriptions and fd_read readability.
pub fn poll_oneoff(ctx: &WasiCtx, mem: &mut [u8],
                   in_ptr: u32, out_ptr: u32, nsubscriptions: u32, nevents_ptr: u32) -> Errno {
    let _ = (ctx, in_ptr);
    let ep = nevents_ptr as usize;
    if ep + 4 > mem.len() { return Errno::Fault; }
    // We signal 0 events (non-blocking), letting the WASM loop spin.
    // A full implementation would block on the Oreulia IPC wait mechanism.
    let _ = (out_ptr, nsubscriptions);
    mem[ep..ep + 4].copy_from_slice(&0u32.to_le_bytes());
    Errno::Success
}

/// sock_recv — receive from a (TCP) socket fd.
pub fn sock_recv(ctx: &mut WasiCtx, mem: &mut [u8], fd: Fd,
                 ri_data_ptr: u32, ri_data_len: u32, _ri_flags: u16,
                 ro_datalen_ptr: u32, ro_flags_ptr: u32) -> Errno {
    let _ = ctx;
    let rdp = ro_datalen_ptr as usize;
    let rfp = ro_flags_ptr as usize;
    if rdp + 4 > mem.len() || rfp + 2 > mem.len() { return Errno::Fault; }
...
pub fn sock_send(_ctx: &mut WasiCtx, mem: &mut [u8], _fd: Fd,
                 si_data_ptr: u32, si_data_len: u32, _si_flags: u16,
                 so_datalen_ptr: u32) -> Errno {
    let sdp = so_datalen_ptr as usize;
    if sdp + 4 > mem.len() { return Errno::Fault; }
    let mut total = 0u32;
...
/// sock_accept — stub (no passive socket support yet).
pub fn sock_accept(_ctx: &mut WasiCtx, _fd: Fd, _flags: u16, _new_fd_ptr: &mut Fd) -> Errno {
    Errno::Notsup
}
