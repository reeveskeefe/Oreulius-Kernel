...
const TEMPORAL_WIFI_HEADER_BYTES: usize =
    TEMPORAL_WIFI_HEADER_V1_BYTES + TEMPORAL_WIFI_REJOIN_BYTES;
...
impl WifiDriver {
    ...
    fn temporal_cache_pmk_for_current_network(&mut self, pmk: &[u8; 32]) {
        ...
        self.temporal_cached_ssid[..len].copy_from_slice(&self.connection.network.ssid[..len]);
    }
    ...
    fn parse_eapol_key_frame(&self, frame: &[u8]) -> Option<ParsedEapolKeyFrame> {
        ...
        let key_info = u16::from_be_bytes([frame[key_desc_offset + 1], frame[key_desc_offset + 2]]);
    ...
    fn verify_eapol_mic(
        ...
        eapol_buf
            .copy_from_slice(&frame[parsed.eapol_offset..parsed.eapol_offset + eapol_total_len]);
    ...
    fn receive_eapol_msg1(&mut self) -> Result<([u8; 32], u64), WifiError> {
        ...
        while crate::pit::get_ticks() < deadline {
            if let Some(len) = self.try_receive_frame(&mut buf)? {
                ...
                if (parsed.key_info
                    & (KI_MIC | KI_SECURE | KI_INSTALL | KI_ENCRYPTED_DATA | KI_ERROR | KI_REQUEST))
                    != 0
                {
                    continue;
                }
        ...
        while crate::pit::get_ticks() < deadline {
            if let Some(len) = self.try_receive_frame(&mut buf)? {
                ...
                let required = KI_DESCRIPTOR_V2
                    ...
                    | KI_ENCRYPTED_DATA;
                let required_mask =
                    KI_VERSION_MASK | KI_PAIRWISE | KI_ACK | KI_MIC | KI_SECURE | KI_ENCRYPTED_DATA;
...
fn temporal_decode_wifi_network(data: &[u8], offset: usize) -> Option<WifiNetwork> {
    ...
    let frequency = temporal_read_u16(data, offset + 4).unwrap_or(0);
    let mut bssid = [0u8; 6];
...
impl WifiDriver {
    ...
    fn record_temporal_state_snapshot(&self) {
        ...
        let payload =
            match self.encode_temporal_state_payload(crate::temporal::TEMPORAL_WIFI_EVENT_STATE) {
                Some(v) => v,
                None => return,
            };
...
pub fn temporal_apply_wifi_driver_payload(payload: &[u8]) -> Result<(), &'static str> {
    ...
    let prior_state =
        temporal_wifi_state_from_u8(payload[6]).ok_or("temporal wifi state invalid")?;
    ...
    if schema == TEMPORAL_WIFI_SCHEMA_V2 {
        ...
        cached_pmk
            .copy_from_slice(&payload[offset + 4 + MAX_SSID_LEN..offset + 4 + MAX_SSID_LEN + 32]);
    ...
    for i in 0..scan_count {
        let net = temporal_decode_wifi_network(payload, offset)
            .ok_or("temporal wifi scan entry invalid")?;
        scan_results[i] = net;
    ...
    let should_resume = enabled
        && (matches!(prior_state, WifiState::Connected | WifiState::Associated)
            || prior_ip_assigned);
