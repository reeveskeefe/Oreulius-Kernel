...
pub fn fd_readdir(_ctx: &mut WasiCtx, mem: &mut [u8], fd: Fd,
...
pub fn path_filestat_get(_ctx: &WasiCtx, mem: &mut [u8],
...
pub fn poll_oneoff(ctx: &WasiCtx, mem: &mut [u8],
    ...
    for i in 0..n {
        ...
        if sub_off + SUB_SIZE > mem.len() { return Errno::Fault; }
        let tag = mem[sub_off + 8];
        if tag == EVENTTYPE_CLOCK {
            let timeout_ns = u64::from_le_bytes(
                mem[sub_off + 24..sub_off + 32].try_into().unwrap_or([0; 8]));
            let flags = u16::from_le_bytes(
                mem[sub_off + 40..sub_off + 42].try_into().unwrap_or([0; 2]));
    ...
    loop {
        ...
        for i in 0..n {
            let sub_off = in_ptr as usize + i * SUB_SIZE;
            if sub_off + SUB_SIZE > mem.len() { break; }
            let tag = mem[sub_off + 8];
    ...
    for i in 0..n {
        ...
        let userdata = u64::from_le_bytes(
            mem[sub_off..sub_off + 8].try_into().unwrap_or([0; 8]));
        ...
        let triggered = match tag {
            ...
            EVENTTYPE_FD_READ => {
                let fd = u32::from_le_bytes(
                    mem[sub_off + 16..sub_off + 20].try_into().unwrap_or([0; 4]));
                if fd == 0 { crate::keyboard::has_input() }
                else       { crate::rtl8139::has_recv() }
            }
        ...
        if triggered {
            ...
            if evt_off + EVT_SIZE > mem.len() { break; }
            // userdata
