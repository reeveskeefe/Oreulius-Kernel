...
impl<const LEVEL: u8> InterruptContext<LEVEL> {
    ...
    pub fn acquire_lock<const TARGET_LEVEL: u8, T, F, R>(&self, lock: &DagSpinlock<TARGET_LEVEL, T>, closure: F) -> R
    where
        ...
        F: FnOnce(&mut T, &InterruptContext<TARGET_LEVEL>) -> R,
    {
        ...
        assert!(TARGET_LEVEL < LEVEL, "DEADLOCK PREVENTED: Attempted to acquire lock of equal or higher DAG priority!");
