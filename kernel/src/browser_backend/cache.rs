...
impl ResponseCache {
    ...
    pub fn lookup(
        ...
        for (i, e) in self.entries.iter().enumerate() {
            ...
            if e.url_digest_len != digest_len { continue; }
            if e.url_digest[..digest_len] != digest[..digest_len] { continue; }
            // TTL check.
            if e.max_age > 0 {
                let age = current_epoch.saturating_sub(e.stored_at);
                if age > e.max_age as u64 { continue; }
            }
            return Some(i);
        }
        None
    ...
    pub fn read_body(&self, idx: usize, out: &mut [u8]) -> usize {
        ...
        if !e.active || e.body_len == 0 { return 0; }
    ...
    pub fn etag_value(&self, idx: usize, out: &mut [u8; 128]) -> usize {
        let e = &self.entries[idx];
        if !e.active || e.etag_len == 0 { return 0; }
    ...
    pub fn last_modified_value(&self, idx: usize, out: &mut [u8; 64]) -> usize {
        let e = &self.entries[idx];
        if !e.active || e.lm_len == 0 { return 0; }
    ...
    pub fn store(
        ...
        if !is_cacheable(status) { return None; }
        if body.len() > MAX_CACHED_BODY { return None; }
    ...
    fn alloc_pool(&mut self, size: usize) -> Option<usize> {
        if size > CACHE_BODY_POOL { return None; }
        ...
        if self.pool_write == 0 && size > 0 { self.pool_write = size; }
        Some(offset)
    }
}
