...
use super::headers::{
    decode_chunked, is_chunked_transfer, parse_content_length, parse_content_type,
    parse_headers, parse_location, parse_status_line,
};
...
pub fn fetch_request<const MAX_EVENTS: usize>(
    ...
    let mut transport = match TransportHandle::connect(ctx.url.scheme, host, port) {
        ...
        Err(TransportError::DnsFailure)       => {
            push_fetch_error(events, event_count, ctx.request, FetchErrorKind::DnsFailure, b"DNS resolution failed");
        ...
        Err(TransportError::TlsAllocFailed) | Err(TransportError::TlsHandshakeFailed) => {
            ...
            push_fetch_error(events, event_count, ctx.request, FetchErrorKind::TlsHandshakeFailed, b"TLS handshake failed");
            return FetchOutcome::Error(FetchErrorKind::TlsHandshakeFailed);
        }
        Err(_) => {
            push_fetch_error(events, event_count, ctx.request, FetchErrorKind::ConnectionFailed, b"Connection failed");
    ...
    if ctx.url.scheme == Scheme::Https {
        push_tls_state(events, event_count, ctx.request, TlsHandshakeResult::Established);
    } else {
        push_tls_state(events, event_count, ctx.request, TlsHandshakeResult::Plaintext);
    }
    ...
    if let Err(_) = transport.send_http_request(
        ...
        transport.close();
        push_fetch_error(events, event_count, ctx.request, FetchErrorKind::ConnectionFailed, b"Send failed");
        return FetchOutcome::Error(FetchErrorKind::ConnectionFailed);
    }
    ...
    if raw_len == 0 {
        transport.close();
        push_fetch_error(events, event_count, ctx.request, FetchErrorKind::ProtocolError, b"No response");
    ...
    if status_code == 0 {
        transport.close();
        push_fetch_error(events, event_count, ctx.request, FetchErrorKind::ProtocolError, b"Bad status line");
        return FetchOutcome::Error(FetchErrorKind::ProtocolError);
    }
    ...
    let content_length = get_header(&headers, header_count, b"content-length")
        .and_then(pcl);
    ...
    // -----------------------------------------------------------------------
    push_headers(
        events, event_count, ctx.request, status_code,
        mime, content_length, &headers, header_count,
    );
    ...
    let outcome = stream_body(
        &mut transport, events, event_count, ctx.request,
...
fn stream_body<const N: usize>(
    ...
    if is_chunked {
        ...
        loop {
            let n = transport.read_raw(&mut recv_buf)
        ...
        loop {
            ...
            let n = transport.read_raw(&mut recv_buf[..remaining])
                .map_err(|_| FetchErrorKind::ConnectionReset)?;
            if n == 0 { break; }
            ...
            while off < n {
                ...
                push_body_chunk(events, event_count, request, &recv_buf[off..off + piece_len], is_last);
                off += piece_len;
            }
...
fn read_until_headers(transport: &mut TransportHandle, buf: &mut [u8; MAX_HEADER_BLOCK]) -> usize {
    ...
    loop {
        ...
        if find_header_end(&buf[..total]).is_some() { break; }
        if total >= MAX_HEADER_BLOCK { break; }
    }
    total
}
...
fn push<const N: usize>(events: &mut [Option<BrowserEvent>; N], count: &mut usize, e: BrowserEvent) {
    if *count < N {
        events[*count] = Some(e);
        *count += 1;
    }
...
fn push_policy_blocked<const N: usize>(
    ...
    push(events, count, BrowserEvent::PolicyBlocked { request_id: req, reason });
}
...
fn push_tls_state<const N: usize>(
    ...
    push(events, count, BrowserEvent::TlsState { request_id: req, result });
}
...
fn push_fetch_error<const N: usize>(
    ...
    push(events, count, BrowserEvent::FetchError { request_id: req, kind, message: msg, msg_len: len });
}
...
fn push_headers<const N: usize>(
    ...
    push(events, count, BrowserEvent::Headers {
        request_id: req,
...
fn push_body_chunk<const N: usize>(
    ...
    push(events, count, BrowserEvent::BodyChunk { request_id: req, data: buf, data_len: len, is_last });
}
