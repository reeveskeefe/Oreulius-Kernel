...
pub const TEMPORAL_CHANNEL_EVENT_SEND_REFUSED: u8 = 4;
pub const TEMPORAL_CHANNEL_EVENT_RECV_REFUSED: u8 = 5;
...
impl TemporalRetentionPolicy {
    const fn default() -> Self {
        Self {
            ...
            max_persist_bytes: self::persistence::MAX_SNAPSHOT_SIZE,
...
impl TemporalService {
    ...
    fn apply_retention_for_persistence_locked(&mut self) {
        let max_bytes = self
            .retention
            .max_persist_bytes
            .min(self::persistence::MAX_SNAPSHOT_SIZE);
...
fn temporal_apply_console_payload(
    ...
    #[cfg(target_arch = "aarch64")]
    {
        let _ = (object_id, owner_pid, write_count, read_count, event);
        Err("temporal console replay unsupported on AArch64")
    }
    #[cfg(not(target_arch = "aarch64"))]
    {
...
fn temporal_apply_network_legacy_payload(
    ...
    {
        let _ = payload;
        Err("temporal legacy network replay unsupported on AArch64")
    }
    #[cfg(not(target_arch = "aarch64"))]
...
fn temporal_store_cap() -> self::persistence::StoreCapability {
    self::persistence::StoreCapability::new(
        0x544D_5054, // "TMPT"
        self::persistence::StoreRights::all(),
    )
}
...
fn persist_state_snapshot() {
    ...
    let mut persistence = self::persistence::persistence().lock();
...
pub fn recover_from_persistence() -> Result<(), &'static str> {
    ...
    let cap = temporal_store_cap();
    let snapshot = {
        let persistence = self::persistence::persistence().lock();
...
pub fn set_retention_policy(
    ...
    svc.retention.max_persist_bytes =
        max_persist_bytes.clamp(256, self::persistence::MAX_SNAPSHOT_SIZE);
    (
        svc.retention.max_versions_per_object,
        svc.retention.max_persist_bytes,
    )
