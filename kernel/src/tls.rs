...
impl TlsSession {
    ...
    fn gen_keys(&mut self) {
        let t = unsafe { crate::pit::get_ticks() };
        let mut seed = [0u8; 16];
    ...
    fn verify_server_finished(&self, verify_data: &[u8], th: &[u8; 32]) -> bool {
        ...
        crate::crypto::ct_eq(&expected, verify_data.try_into().unwrap_or(&[0u8; 32]))
    }
    ...
    pub fn tick(&mut self) {
        let mut frame = [0u8; 1600];
        let n = crate::rtl8139::recv(&mut frame);
