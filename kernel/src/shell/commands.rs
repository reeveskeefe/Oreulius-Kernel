...
pub fn execute(input: &str) {
    ...
    match command {
        "help" => {
            ...
            vga::print_str("  ipc-list   - List active IPC channels\n");
            vga::print_str("  ipc-inspect - Inspect a channel (usage: ipc-inspect <chan>)\n");
            ...
            vga::print_str("  ipc-selftest - Run deterministic IPC self-test suite\n");
            ...
            vga::print_str("  fork-test      - Run x86_64 fork/COW user-mode regression\n");
            vga::print_str("  elf-run       - Load and run ELF from VFS (elf-run <path>)\n");
            vga::print_str("  wasm           - Load and run WASM binary from VFS (wasm <path>)\n");
            vga::print_str("\nFile Utilities (POSIX aliases):\n");
            vga::print_str("  ls     <path>              - List directory (default: /)\n");
            vga::print_str("  cat    <path>              - Print file contents\n");
            vga::print_str("  mkdir  <path>              - Create directory\n");
            vga::print_str("  rm     <path>              - Delete file or empty directory\n");
            vga::print_str("  cp     <src> <dst>         - Copy file\n");
            vga::print_str("  stat   <path>              - Show file size and metadata\n");
            vga::print_str("\nOperational Tooling:\n");
            vga::print_str("  health       - Show live system health snapshot\n");
            vga::print_str("  health-history - Show stored health snapshot history\n");
        ...
        // ── POSIX-style file utilities (aliases over VFS) ────────────────────
        "ls" => {
            cmd_vfs_ls(parts);
        }
        "cat" => {
    ...
    match crate::temporal::snapshot_path(path) {
        ...
        Err(e) => match e {
            crate::temporal::TemporalError::VfsReadFailed => {
                ...
                vga::print_str(path);
                vga::print_str(
                    " was not readable from VFS. Use the exact same path you wrote first.\n",
                );
...
fn temporal_merge_strategy_from_str(value: &str) -> Option<crate::temporal::TemporalMergeStrategy> {
    match value {
        ...
        "three-way" | "threeway" | "diff3" => {
            Some(crate::temporal::TemporalMergeStrategy::ThreeWay)
        }
        _ => None,
    }
}
...
fn temporal_ipc_parse_response_frame(
    ...
    let session_id =
        temporal_ipc_read_u64(frame, 0).ok_or("temporal IPC response missing session")?;
    let frame = &frame[TEMPORAL_IPC_SESSION_BYTES..];
...
fn temporal_ipc_service_self_check() -> Result<(), &'static str> {
    ...
    let merge_req =
        temporal_ipc_build_request_frame(session_id, TEMPORAL_IPC_OP_MERGE, 0, 7, &merge_payload)?;
    ...
    let checkout_req = temporal_ipc_build_request_frame(
        session_id,
...
fn cmd_temporal_ipc_demo() {
    ...
    let stats_req =
        match temporal_ipc_build_request_frame(session_id, TEMPORAL_IPC_OP_STATS, 0, 101, &[]) {
            Ok(v) => v,
            Err(e) => {
...
// ============================================================================
// WASM Loader — `wasm <path>`
// ============================================================================

fn cmd_wasm_run(mut parts: core::str::SplitWhitespace) {
    const POST_MAIN_THREAD_DRAIN_QUANTA: usize = 4096;

    let path = match parts.next() {
        Some(p) => p,
        None => {
            vga::print_str("Usage: wasm <path>\n");
            return;
        }
    };

    // 1. Determine file size so we can allocate the right buffer.
    let size = match vfs::path_size(path) {
        Ok(n) => n,
        Err(e) => {
            vga::print_str("wasm: stat failed for ");
            vga::print_str(path);
            vga::print_str(": ");
            vga::print_str(e);
            vga::print_str("\n");
    ...
    if size == 0 {
        vga::print_str("wasm: empty file: ");
        vga::print_str(path);
        vga::print_str("\n");
        return;
    }

    // 2. Read bytes from VFS.
    let mut buf = alloc::vec::Vec::new();
    buf.resize(size, 0u8);
    let n = match vfs::read_path(path, &mut buf) {
        Ok(n) => n,
        Err(e) => {
            vga::print_str("wasm: read failed: ");
            vga::print_str(e);
            vga::print_str("\n");
            return;
        }
    ...
    // 3. Get or create a PID for this module.
    let pid = match process::current_pid() {
        Some(p) => p,
        None => {
            vga::print_str("wasm: no current process\n");
            return;
        }
    };

    // 4. Instantiate the module.
    let instance_id = match wasm::wasm_runtime().instantiate(&buf, pid) {
        Ok(id) => id,
        Err(e) => {
            vga::print_str("wasm: instantiate failed: ");
            vga::print_str(e.as_str());
            vga::print_str("\n");
            return;
        }
    ...
    vga::print_str("wasm: running ");
    vga::print_str(path);
    vga::print_str(" (instance ");
    print_usize(instance_id);
    vga::print_str(")\n");

    // 5. Call the entry point (function index 0 / _start).
    let result = wasm::wasm_runtime().get_instance_mut(instance_id, |inst| inst.call(0));
    let main_returned_ok = match result {
        Ok(Ok(_)) => {
            vga::print_str("wasm: exited OK\n");
            true
        }
        Ok(Err(e)) => {
            vga::print_str("wasm: runtime error: ");
            vga::print_str(e.as_str());
            vga::print_str("\n");
            false
        }
        Err(e) => {
            vga::print_str("wasm: instance error: ");
            vga::print_str(e.as_str());
            vga::print_str("\n");
            false
        }
    ...
    // 6. Drain any child WASM processes spawned during execution.
    wasm::drain_pending_spawns();

    if main_returned_ok {
        match wasm::wasm_runtime()
            .drain_instance_background_threads(instance_id, POST_MAIN_THREAD_DRAIN_QUANTA)
        {
            Ok(wasm::BackgroundThreadDrainOutcome::Drained { quanta }) => {
                if quanta != 0 {
                    vga::print_str("wasm: drained cooperative threads (quanta ");
                    print_usize(quanta);
                    vga::print_str(")\n");
                }
            }
            Ok(wasm::BackgroundThreadDrainOutcome::Stalled { quanta, status }) => {
                vga::print_str("wasm: warning: background threads stalled before teardown");
                if quanta != 0 {
                    vga::print_str(" after ");
                    print_usize(quanta);
                    vga::print_str(" quanta");
                }
                vga::print_str(" (live/run/join/yld/fin ");
                print_u32(status.live);
                vga::print_str("/");
                print_u32(status.runnable);
                vga::print_str("/");
                print_u32(status.joining);
                vga::print_str("/");
                print_u32(status.yielded);
                vga::print_str("/");
                print_u32(status.finished);
                vga::print_str(")\n");
            }
            Ok(wasm::BackgroundThreadDrainOutcome::TimedOut { quanta, status }) => {
                vga::print_str("wasm: warning: background thread drain hit budget ");
                print_usize(quanta);
                vga::print_str(" (live/run/join/yld/fin ");
                print_u32(status.live);
                vga::print_str("/");
            ...
            Err(e) => {
                vga::print_str("wasm: background thread drain failed: ");
                vga::print_str(e.as_str());
                vga::print_str("\n");
            }
        }
    } else if let Ok(status) = wasm::wasm_runtime().thread_pool_status(instance_id) {
        if status.live != 0 {
            vga::print_str("wasm: warning: tearing down instance with live background threads (");
            print_u32(status.live);
            vga::print_str(")\n");
        }
    }

    // 7. Destroy the instance to free resources.
    let _ = wasm::wasm_runtime().destroy(instance_id);
}

// ============================================================================
// POSIX-style file utility commands (thin wrappers over VFS)
// ============================================================================

/// `cat <path>` — print file contents as text (or hex hint for binary)
fn cmd_cat(mut parts: core::str::SplitWhitespace) {
    let path = match parts.next() {
        Some(p) => p,
        None => {
            vga::print_str("Usage: cat <path>\n");
            return;
        }
    };
...
fn cmd_rm(mut parts: core::str::SplitWhitespace) {
    let path = match parts.next() {
        Some(p) => p,
        None => {
            vga::print_str("Usage: rm <path>\n");
            return;
        }
    };
    ...
    match vfs::unlink(path) {
        ...
        Err(_) => {
            ...
            match vfs::rmdir(path) {
                Ok(()) => {
                    vga::print_str("removed directory ");
                    vga::print_str(path);
                    vga::print_str("\n");
                }
                Err(e) => {
...
fn cmd_cp(mut parts: core::str::SplitWhitespace) {
    let src = match parts.next() {
        Some(p) => p,
        None => {
            vga::print_str("Usage: cp <src> <dst>\n");
            return;
        }
    };
    ...
    let mut buf = alloc::vec::Vec::new();
    buf.resize(size.max(1), 0u8);
    let n = match vfs::read_path(src, &mut buf) {
        Ok(n) => n,
        Err(e) => {
            vga::print_str("cp: read ");
            vga::print_str(src);
            vga::print_str(": ");
            vga::print_str(e);
            vga::print_str("\n");
...
/// `stat <path>` — show file size and VFS metadata
fn cmd_stat(mut parts: core::str::SplitWhitespace) {
    let path = match parts.next() {
        Some(p) => p,
        None => {
            vga::print_str("Usage: stat <path>\n");
            return;
        }
    };

    match vfs::path_size(path) {
        Ok(size) => {
            vga::print_str("  File: ");
            vga::print_str(path);
            vga::print_str("\n  Size: ");
...
fn demo_channel_caps(chan_id: u32) -> Option<(ipc::ChannelCapability, ipc::ChannelCapability)> {
    unsafe {
        DEMO_CHANNELS
            .iter()
            .find(|(id, _)| *id == chan_id)
            .and_then(|(_, caps)| *caps)
    }
}
...
fn print_ipc_flag_bits(bits: u32) {
    ...
    if (bits & ipc::ChannelFlags::BOUNDED) != 0 {
        vga::print_str("bounded");
        wrote_any = true;
    }
    if (bits & ipc::ChannelFlags::UNBOUNDED) != 0 {
        if wrote_any {
            vga::print_str("|");
        }
        vga::print_str("unbounded");
        wrote_any = true;
    }
    if (bits & ipc::ChannelFlags::HIGH_PRIORITY) != 0 {
        if wrote_any {
            vga::print_str("|");
        }
        vga::print_str("highprio");
        wrote_any = true;
    }
    if (bits & ipc::ChannelFlags::RELIABLE) != 0 {
        if wrote_any {
            vga::print_str("|");
        }
        vga::print_str("reliable");
        wrote_any = true;
    }
    if (bits & ipc::ChannelFlags::ASYNC) != 0 {
        if wrote_any {
            vga::print_str("|");
        }
        vga::print_str("async");
...
fn print_ipc_backpressure_action(action: ipc::BackpressureAction) {
    vga::print_str(action.as_str());
}
...
fn cmd_ipc_send(mut parts: core::str::SplitWhitespace) {
    ...
    let send_cap = demo_channel_caps(chan_id).map(|(send, _)| send);
...
fn cmd_ipc_recv(mut parts: core::str::SplitWhitespace) {
    ...
    let recv_cap = demo_channel_caps(chan_id).map(|(_, recv)| recv);
...
fn cmd_ipc_list() {
    let diag = ipc::ipc().diagnostics();
    vga::print_str("IPC channel list:\n");
    vga::print_str("  active: ");
    print_number(diag.active_channels);
    vga::print_str(" / ");
    print_number(diag.max_channels);
    vga::print_str("\n");

    if diag.active_channels == 0 {
        vga::print_str("  <none>\n");
        return;
    }

    for slot in diag.channels.iter() {
        let Some(channel) = slot else {
            continue;
        };
        vga::print_str("  channel=");
        print_u32(channel.id.0);
        vga::print_str(" creator=");
        print_u32(channel.creator.0);
        vga::print_str(" pending=");
        print_number(channel.pending);
        vga::print_str("/");
        ...
        print_u32(channel.send_refusals);
        vga::print_str("/");
        print_u32(channel.recv_refusals);
        vga::print_str(" pressure=");
        print_ipc_backpressure(channel.pressure);
        vga::print_str("/");
        print_ipc_backpressure_action(channel.pressure_action);
        vga::print_str(" hw=");
        print_number(channel.high_watermark);
        vga::print_str(" hits=");
        print_u32(channel.high_pressure_hits);
        vga::print_str("/");
        print_u32(channel.saturated_hits);
        vga::print_str(" wakes=");
        print_u32(channel.receiver_wakeups);
        vga::print_str("/");
        print_u32(channel.sender_wakeups);
        vga::print_str(" wait=");
        print_number(channel.waiting_receivers);
        vga::print_str("/");
        print_number(channel.waiting_senders);
        vga::print_str(" tracked=");
        vga::print_str(if demo_channel_caps(channel.id.0).is_some() {
            "yes"
        } else {
            "no"
        });
...
fn cmd_ipc_inspect(mut parts: core::str::SplitWhitespace) {
    let chan_id = match parts.next().and_then(parse_number) {
        Some(n) => n as u32,
        None => {
            vga::print_str("Usage: ipc-inspect <channel_id>\n");
            return;
        }
    };

    match ipc::ipc().inspect_channel(ipc::ChannelId::new(chan_id)) {
        Ok(channel) => {
            vga::print_str("IPC channel ");
            print_u32(channel.id.0);
            vga::print_str("\n  creator: ");
            print_u32(channel.creator.0);
            vga::print_str("\n  pending: ");
            ...
            vga::print_str("\n  full: ");
            vga::print_str(if channel.full { "yes" } else { "no" });
            vga::print_str("\n  priority: ");
            print_u8_val(channel.priority);
            vga::print_str("\n  send refusals: ");
            print_u32(channel.send_refusals);
            ...
            print_number(channel.waiting_senders);
            vga::print_str("\n  flags: ");
            print_ipc_flag_bits(channel.flags_bits);
            vga::print_str("\n  demo tracked: ");
            vga::print_str(if demo_channel_caps(chan_id).is_some() {
                "yes"
            } else {
                "no"
            });
        ...
        Err(e) => {
            vga::print_str("IPC inspect error: ");
            vga::print_str(e.as_str());
            vga::print_str("\n");
        }
    }
...
fn cmd_ipc_stats() {
    let diag = ipc::ipc().diagnostics();
    let mut send_refusals = 0u32;
    let mut recv_refusals = 0u32;
    let mut max_high_watermark = 0usize;
    let mut high_pressure_hits = 0u32;
    let mut saturated_hits = 0u32;
    let mut receiver_wakeups = 0u32;
    let mut sender_wakeups = 0u32;
    let mut waiting_receivers = 0usize;
    let mut waiting_senders = 0usize;
    for slot in diag.channels.iter() {
        let Some(channel) = slot else {
            continue;
        };
        send_refusals = send_refusals.saturating_add(channel.send_refusals);
        recv_refusals = recv_refusals.saturating_add(channel.recv_refusals);
        max_high_watermark = max_high_watermark.max(channel.high_watermark);
        high_pressure_hits = high_pressure_hits.saturating_add(channel.high_pressure_hits);
        saturated_hits = saturated_hits.saturating_add(channel.saturated_hits);
        receiver_wakeups = receiver_wakeups.saturating_add(channel.receiver_wakeups);
        sender_wakeups = sender_wakeups.saturating_add(channel.sender_wakeups);
        waiting_receivers = waiting_receivers.saturating_add(channel.waiting_receivers);
        waiting_senders = waiting_senders.saturating_add(channel.waiting_senders);
    }
    ...
    print_number(diag.max_channels);
    vga::print_str("\n");
    vga::print_str("  Demo slots tracked: ");
    print_number(count_demo_channels());
    vga::print_str("\n");
    vga::print_str("  Send refusals: ");
    print_u32(send_refusals);
    vga::print_str("\n");
    vga::print_str("  Recv refusals: ");
    print_u32(recv_refusals);
    vga::print_str("\n");
    vga::print_str("  Max high watermark: ");
    print_number(max_high_watermark);
    vga::print_str("\n");
    vga::print_str("  High pressure hits: ");
    print_u32(high_pressure_hits);
    vga::print_str("\n");
    vga::print_str("  Saturated hits: ");
    print_u32(saturated_hits);
    vga::print_str("\n");
    vga::print_str("  Receiver wakeups: ");
    print_u32(receiver_wakeups);
    vga::print_str("\n");
    vga::print_str("  Sender wakeups: ");
    print_u32(sender_wakeups);
    vga::print_str("\n");
    vga::print_str("  Waiting receivers: ");
    print_number(waiting_receivers);
    vga::print_str("\n");
    vga::print_str("  Waiting senders: ");
    print_number(waiting_senders);
    vga::print_str("\n");
}

fn cmd_ipc_selftest() {
    let report = ipc::run_selftest();
    vga::print_str("IPC self-test:\n");
    for case in report.cases.iter() {
        ...
        vga::print_str(": ");
        vga::print_str(if case.passed { "PASS" } else { "FAIL" });
    ...
    vga::print_str("  Summary: ");
    print_number(report.passed);
    vga::print_str(" / ");
    print_number(report.total);
    vga::print_str("\n");
}
...
fn handle_temporal_request(message: &ipc::Message) -> ipc::Message {
    ...
    let (session_id, opcode, flags, request_id, payload) =
        match temporal_ipc_parse_request_frame(frame) {
            Ok(parsed) => parsed,
            Err(status) => {
    ...
    temporal_ipc_build_response_frame(
        session_id,
        opcode,
        flags,
        request_id,
...
fn cmd_svc_request(mut parts: core::str::SplitWhitespace) {
    ...
    let intro_response = registry::registry().introduce(
        IntroductionRequest::new(service_type, caller_pid),
        &mut introducer,
    );
    ...
    let response_msg = match ipc::ipc().try_recv(&svc_recv_cap) {
        Ok(incoming) => {
            ...
            inner.payload[..inner_len].copy_from_slice(&incoming.payload[4..4 + inner_len]);
...
fn cmd_wasm_list() {
    ...
    vga::print_str("---------------\n");
    vga::print_str("ID   PID   Status   Thr Live/Run/Join/Yld/Fin   Spawned\n");
    ...
    for (id, pid, active) in instances.iter() {
        if *active {
            ...
            print_u32(pid.0);
            vga::print_str("    Active   ");
            match wasm::wasm_runtime().thread_pool_status(*id) {
                Ok(status) => {
                    print_u32(status.live);
                    vga::print_str("/");
                    print_u32(status.runnable);
                    ...
                    print_u32(status.finished);
                    vga::print_str("                 ");
                    print_u32(status.total_spawned);
                    vga::print_str("\n");
                }
                Err(_) => {
                    vga::print_str("n/a                               n/a\n");
                }
            }
            count += 1;
        }
...
fn cmd_prod_bench() {
    ...
    crate::serial_println!(
        "[PROD-BENCH] cpu nop_cpo={} add_cpo={} status=ok",
        nop_cpo,
        add_cpo
    );
    passed += 1;
...
struct SharedJitRecover;

impl Drop for SharedJitRecover {
    fn drop(&mut self) {
...
fn run_with_shared_jit_fuzz_context<T>(user_mode: bool, f: impl FnOnce() -> T) -> T {
    ...
    // races while bouncing through the ring transition path. Quiesce
    // IRQ-driven preemption for this command run to keep results stable.
    let _irq_quiesce = SharedFuzzIrqQuiesce::maybe_enter(user_mode);
    f()
}
...
fn cmd_wasm_jit_fuzz(mut parts: core::str::SplitWhitespace) {
    ...
    let stats_result = if cfg!(target_arch = "x86_64") && !user_mode && iters > X64_FUZZ_CHUNK_ITERS
    {
        ...
        run_with_shared_jit_fuzz_context(user_mode, || {
            crate::execution::wasm::jit_fuzz(iters, seed)
        })
    };
...
fn cmd_jitfuzzreg(mut parts: core::str::SplitWhitespace) {
    let arg1 = parts.next();
    let arg2 = parts.next();
    let arg3 = parts.next();
    if arg3.is_some() {
        vga::print_str("Usage: jitfuzzreg [iters [seeds]] | jitfuzzreg full [iters]\n");
        return;
    }

    let mut iterations_per_seed: u32 = 1;
    let mut seeds_limit: u32 = 2;
    let mut full = false;
    ...
    if let Some(a1) = arg1 {
        if a1 == "full" {
            full = true;
            if let Some(a2) = arg2 {
                if let Some(v) = parse_number(a2) {
                    iterations_per_seed = (v as u32).max(1);
                } else {
                    vga::print_str("Usage: jitfuzzreg [iters [seeds]] | jitfuzzreg full [iters]\n");
                    return;
                }
        ...
        vga::print_str("Usage: jitfuzzreg [iters [seeds]] | jitfuzzreg full [iters]\n");
        vga::print_str("Example: jitfuzzreg full\n");
        return;
    }

    const MAX_FUZZ_ITERS: u32 = 10_000;
    if iterations_per_seed == 0 || iterations_per_seed > MAX_FUZZ_ITERS {
        ...
        if full {
            ...
            match crate::execution::wasm::jit_fuzz_regression_default(iterations_per_seed) {
                Ok(stats) => {
                    ...
                    vga::print_str(" seeds_failed=");
                    print_u32(stats.seeds_failed);
                    vga::print_str(" mismatches=");
                    print_u32(stats.total_mismatches);
                    vga::print_str(" compile_errors=");
                    print_u32(stats.total_compile_errors);
                    vga::print_str(" edges_full=");
                    print_u32(stats.max_opcode_edges_hit);
                    vga::print_str("/");
                    print_u32(edges_total);
                    vga::print_str(" edges_adm=");
                ...
                Err(e) => {
                    vga::print_str("[jitfuzzreg] failed: ");
                    vga::print_str(e);
                    vga::print_str("\n");
                }
            }
        } else {
            vga::print_str("[jitfuzzreg] mode: bounded\n");
            vga::print_str("[jitfuzzreg] iterations/seed: ");
            print_u32(iterations_per_seed);
            vga::print_str(" seeds: ");
            print_u32(seeds_limit);
            vga::print_str("\n");
            match crate::execution::wasm::jit_fuzz_regression_bounded(
                iterations_per_seed,
                seeds_limit,
            ) {
                Ok(stats) => {
...
fn cmd_capnet_fuzz_soak(mut parts: core::str::SplitWhitespace) {
    ...
    match run_capnet_with_irqs_masked(|| {
        crate::net::capnet::capnet_fuzz_regression_soak_default(iters, rounds)
    }) {
        Ok(stats) => {
            vga::print_str("Rounds passed: ");
            print_u32(stats.rounds_passed);
...
fn cmd_capnet_demo() {
    ...
    if let Err(e) = crate::net::capnet::register_peer(
        loopback_peer,
    ...
    if let Err(e) =
        crate::net::capnet::install_peer_session_key(loopback_peer, key_epoch, k0, k1, 0)
    {
    ...
    let revoke = match crate::net::capnet::build_token_revoke_frame(
        loopback_peer,
        offer.seq,
        offer.token_id,
    ) {
...
fn cmd_sched_stats() {
    ...
    #[cfg(target_arch = "x86_64")]
    {
        ...
        vga::print_str("Processes:\n  Total:    ");
        print_usize(snapshot.total_processes);
        vga::print_str("\n  Running:  ");
        print_usize(snapshot.running_processes);
        vga::print_str("\n  Ready:    ");
        print_usize(snapshot.ready_processes);
        vga::print_str("\n  Sleeping: ");
        print_usize(snapshot.sleeping_processes);
        vga::print_str("\n\nContext Switches:\n  Total:       ");
        print_u64(snapshot.total_switches);
        vga::print_str("\n  Preemptions: ");
...
fn cmd_sched_net_soak(mut parts: core::str::SplitWhitespace) {
    ...
    let sched_before = crate::scheduler::slice_scheduler::scheduler()
        .lock()
        .get_stats();
    ...
    let sched_after = crate::scheduler::slice_scheduler::scheduler()
        .lock()
        .get_stats();
