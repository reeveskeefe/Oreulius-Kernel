...
impl AesNi {
    ...
    pub fn encrypt_block(output: &mut [u8; 16], input: &[u8; 16], key_schedule: &[u8], rounds: u32) {
        unsafe {
            aes_encrypt_block(
                output.as_mut_ptr(),
                input.as_ptr(),
    ...
    pub fn decrypt_block(output: &mut [u8; 16], input: &[u8; 16], key_schedule: &[u8], rounds: u32) {
        unsafe {
            aes_decrypt_block(
                output.as_mut_ptr(),
                input.as_ptr(),
