...
impl Default for ThreadValue {
    fn default() -> Self { ThreadValue::I32(0) }
}
...
impl WasmThreadPool {
    ...
    pub fn spawn(&mut self, func_idx: u32, arg: i32, initial_pc: usize) -> Result<i32, &'static str> {
        if !self.initialized {
        ...
        // Find an empty slot.
        let slot = self.threads.iter().position(|t| matches!(t.state, ThreadState::Empty))
    ...
    if let Some(reg) = POOL_REGISTRY.try_lock() {
        for slot in reg.entries.iter() {
            if let Some(entry) = slot {
                ...
                // Advance the round-robin one step: mark yielded→runnable and
                // refill fuel.  The actual bytecode execution happens in the
                // WasmRuntime's normal step loop.
                if let Some(t) = pool.next_runnable() {
                    if matches!(t.state, ThreadState::Yielded) {
                        t.state = ThreadState::Runnable;
                        t.fuel = DEFAULT_THREAD_FUEL;
                    }
