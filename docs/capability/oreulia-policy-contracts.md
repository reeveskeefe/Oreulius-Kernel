...
| **Fail-open when no policy** | A capability without a bound policy behaves identically to a classic capability. Backward compatible. |
...
│  policy::query(cap_id)            → Option<PolicyInfo>              │
└─────────────────────────────┬────────────────────────────────────────┘
...
└──────────┬──────────────────────────────────────────────────────────┘
    ...
    │                                                               │
    │  Mode 1: Full WASM engine (fail-open in current impl)        │
    └───────────────────────────────────────────────────────────────┘
...
**Definition I.2 (Fail-Open Semantics).** When no policy is bound to $(p, c)$, the policy function is treated as the constant function $\lambda\_.\, \text{Permit}$. This preserves backward compatibility with classic capability semantics.
...
**Definition I.9 (Hash Binding Integrity Check).** A caller can query the stored hash via `policy_query` (ID 124) and compare against a freshly computed hash of the expected bytecode to verify the bound policy has not been replaced.
...
fn run_policy_contract(bytecode: &[u8], ctx: &[u8]) -> bool {
    ...
    // Mode 1: full WASM engine — fail-open in current implementation
    true
}
```
...
pub fn policy_check_for_cap(pid: u32, cap_id: u32, ctx: &[u8]) -> bool {
    ...
    match policy {
        None    => true,   // no policy bound → fail-open
...
After unbinding, subsequent capability accesses on `cap_id` fail-open (Invariant I.2).
...
4. Returns `0` (Permit), `1` (Deny), or `−1` (no policy bound).

This host function allows WASM modules to explicitly evaluate their own policies before invoking an operation, enabling proactive denial without waiting for the kernel hot path to reject.
...
match policy::eval(my_cap_id, &ctx) {
    ...
    PolicyResult::NoPolicyBound => { /* no policy: fail-open */ }
}

// ── Query bound policy metadata ─────────────────────────────────────────────
if let Some(info) = policy::query(my_cap_id) {
    let hash = info.hash;
...
// ── SDK types ───────────────────────────────────────────────────────────────
pub enum PolicyResult { Permit, Deny, NoPolicyBound }
...
Bind a policy to a FS_WRITE capability that only permits access during business hours. Context bytes encode the current hour (0–23). The OPOL stub checks `ctx[0] >= 8 && ctx[0] < 18`. Since OPOL is single-byte, the hour must be represented as a single byte; two-range checks require a short WASM program (full WASM engine, currently fail-open).
...
- If not found (common case): unlock immediately, return `true`
...
| Limitation | Detail |
|---|---|
| **Full WASM engine fail-open** | Mode 1 (arbitrary WASM bytecode) currently returns `true` (permit) unconditionally. A real WASM interpreter must be integrated before arbitrary contracts have enforcement value. |
...
| **No policy delegation** | When a capability is delegated (transferred to another process), the policy binding is **not** automatically copied. The delegatee receives a capability with no policy (fail-open). |
...
| **OPOL single-byte comparison** | The OPOL stub supports exactly one context byte comparison. More complex logic (range checks, multi-byte comparisons) requires full WASM mode (currently fail-open). |
